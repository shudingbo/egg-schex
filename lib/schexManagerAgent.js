...
YZSchedule.prototype.evtInit = function(evtInfo){
	...
	if( evtInfo.err !== null ){
		this.logger.warn(`job ${evtInfo.name} init err:${evtInfo.msg}`);
...
YZSchedule.prototype.evtRun = function(evtInfo){
	...
	if( evtInfo.err !== null ){
		this.logger.warn(`job ${evtInfo.name} run err:${evtInfo.msg}`);
		this.drv.update_Job( con.STA.EXCEPTION, job);
		return;
	}
...
YZSchedule.prototype.evtStop = function(evtInfo){
	...
	if( evtInfo.err !== null ){
		this.logger.warn(`job ${evtInfo.name} stop err:${evtInfo.msg}`);
		this.drv.update_Job( con.STA.EXCEPTION, job);
		return;
	}
...
function runJob(self, job ){
	if( job.status === false){
		if( job.parent === null || job.parent === undefined )
		{
			...
			self.runAction( job, con.JobStep.INIT ,{
				fun : job.fun,
				parent:null,
				cfg : self.getConfig( job.name )
			});
		}
	}
}
...
function stopJob( self, job ){
	if( job.status === true){
		...
		if( job.job !== null ){
			...
			console.log('stop',self.schedules[ job.name ]);
			self.schedules[ job.name ] = null;
		}
	}
}
