...
class SchexManagerAgent {
  ...
  evtInit(evtInfo) {
    ...
    // /
    const jobCfg = { rule: job.cron };
    if ( typeof(job.tz) === 'string' && job.tz.length > 0 ) {
      jobCfg.tz = job.tz;
    }

    job.job = schedule.scheduleJob(jobCfg, () => {
      this.runAction(job, con.JobStep.RUN, {});
    });
    ...
    this.drv.update_Job(con.STA.START, job);

    if (job.parent !== null && job.parent !== undefined) {
      this.logger.info(`[schex-ag] run Sub Job [ ${job.name} ]`);
    } else {
      this.logger.info(`[schex-ag] run Job [ ${job.name} ]`);
    }
  }
  ...
  onEvtCtl(evtInfo) {
    let job = null;
    if (evtInfo.method !== con.Method.ctlSta && evtInfo.method !== con.Method.ctlAddJob) {
    ...
    switch (evtInfo.method) {
      case con.Method.ctlSta:
        ...
        break;
      case con.Method.ctlAddJob:
        if (this.schedules[ evtInfo.name ] !== undefined) {
          evtInfo.status = false;
          evtInfo.msg = `${evtInfo.name} has exists！`;
          this.agent.messenger.sendTo(evtInfo.pid, 'egg-schex', evtInfo);
          return;
        }
        ...
        (async () => {
          try {
            const retMsg = await this.drv.addJob(evtInfo);
            evtInfo.msg = retMsg;
            evtInfo.status = true;
            this.agent.messenger.sendTo(evtInfo.pid, 'egg-schex', evtInfo);
          } catch (e) {
      ...
      case con.Method.ctlDelJob:
        if (job.status === true) {
          evtInfo.status = false;
          evtInfo.msg = `${evtInfo.name} is running！Can't delete.`;
          this.agent.messenger.sendTo(evtInfo.pid, 'egg-schex', evtInfo);
          return;
        }
        ...
        (async () => {
          try {
            const retMsg = await this.drv.deleteJob(evtInfo.name);
            evtInfo.msg = retMsg;
            evtInfo.status = true;
            this.agent.messenger.sendTo(evtInfo.pid, 'egg-schex', evtInfo);

            for (const it in this.schedules) {
              if (this.schedules[it].parent !== null && this.schedules[it].parent.name === evtInfo.name) {
                delete this.schedules[it];
              }
            }

            delete this.schedules[evtInfo.name];
          } catch (e) {
            evtInfo.status = false;
            evtInfo.msg = e.toString();
            this.agent.messenger.sendTo(evtInfo.pid, 'egg-schex', evtInfo);
      ...
      case con.Method.ctlUpdateJob:
        (async () => {
          try {
            const retMsg = await this.drv.updateJob(evtInfo);
            evtInfo.msg = retMsg;
            evtInfo.status = true;
            this.agent.messenger.sendTo(evtInfo.pid, 'egg-schex', evtInfo);
          } catch (e) {
...
function addSubJob(self, name, scCfg) {
  ...
  if (self.schedules[ name ] === undefined) {
    const jobTmp = { name,
      cron: scCfg.cron,
      tz: (scCfg.tz)?scCfg.tz:jobParent.tz,
      fun: scCfg.fun,
      switch: scCfg.switch,
      status: false,
