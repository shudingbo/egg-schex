...
YZSchedule.prototype.onEvtRun = function (eggctx, evtInfo )
{
	...
	try{
		let msg = job.ins(eggctx, this,job, con.JobStep.RUN );
		if( msg !== undefined ){
			job.msg = msg;
		}
...
YZSchedule.prototype.onEvtStop = function (eggctx, evtInfo )
{
	let job = this.jobs[ evtInfo.name ];
	job.ctx = evtInfo.ctx;

	try{
		let msg = job.ins(eggctx, this,job, con.JobStep.STOP );
		if( msg !== undefined ){
			job.msg = msg;
		}
