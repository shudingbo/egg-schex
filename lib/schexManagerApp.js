...
YZSchedule.prototype.onJobEvt = function(evtInfo){
  this.logger.info( 'schex app recv:',evtInfo );
  ...
  if( this.jobs[evtInfo.name] === undefined){// create instance
		...
		if( job.parent === null || job.parent === undefined )
		{
			...
			let ins = require( pwd );
			job.ins = ins;
			let ctx = job.ins( this,job, con.JobStep.INIT );
			if( ctx === undefined ){
				ctx = {};
			}
  ...
  switch( evtInfo.step ){
		...
		case this.jobStep.RUN:
			{
				this.onEvtRun(evtInfo);
			}break;
		case this.jobStep.RUN:
			{
				this.onEvtStop(evtInfo);
			}break;
  }
};
...
YZSchedule.prototype.onEvtRun = function ( evtInfo )
{
	...
	try{
		let msg = job.ins( this,job, con.JobStep.RUN );
		if( msg !== undefined ){
			job.msg = msg;
		}

		this.sendMsg( job, con.JobStep.RUN, null);
	}catch(e){
		this.sendMsg( job, con.JobStep.RUN,e.toString());
	}
};

YZSchedule.prototype.onEvtStop = function ( evtInfo )
{
	let job = this.jobs[ evtInfo.name ];
	...
	try{
		let msg = job.ins( this,job, con.JobStep.STOP );
		if( msg !== undefined ){
			job.msg = msg;
		}
		...
		this.sendMsg( job, con.JobStep.STOP,e.toString());
	}
};
